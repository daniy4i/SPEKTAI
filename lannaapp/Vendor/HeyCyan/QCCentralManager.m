...
#if TARGET_OS_IOS && __has_include(<QCSDK/QCSDKManager.h>)
...
- (instancetype)init
{
    ...
    if (self) {
        NSMutableDictionary *options = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                         ...
                                         @YES,CBConnectPeripheralOptionNotifyOnConnectionKey,
                                         nil];

#if TARGET_OS_IOS
        id backgroundModes = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"UIBackgroundModes"];
        BOOL supportsBluetoothBackground = NO;
        if ([backgroundModes isKindOfClass:[NSArray class]]) {
            supportsBluetoothBackground = [(NSArray *)backgroundModes containsObject:@"bluetooth-central"];
        } else if ([backgroundModes isKindOfClass:[NSString class]]) {
            supportsBluetoothBackground = [backgroundModes isEqualToString:@"bluetooth-central"];
        }

        if (supportsBluetoothBackground) {
...
- (void)forgetLastConnectedPeripheral {
    ...
    if (self.connectedPeripheral) {
        @try {
            ...
            NSLog(@"Failed to cancel connection: %@", exception);
        }
    }
...
- (instancetype)init {
    self = [super init];
    if (self) {
        _deviceState = QCStateUnbind;
        _bleState = QCBluetoothStateUnsupported;
    }
    return self;
}
