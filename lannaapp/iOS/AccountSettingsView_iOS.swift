...
struct AccountSettingsView_iOS: View {
        ...
        NavigationView {
            VStack(spacing: DS.spacingXL) {
                ...
                VStack(spacing: DS.spacingM) {
                    ...
                    Button(action: { showingIntegrations = true }) {
                        HStack {
            ...
            .sheet(isPresented: $showingReauthSheet) {
                ReauthSheetView(email: authService.user?.email ?? "", password: $reauthPassword) {
...
private struct ReauthSheetView: View {
    let email: String
...
extension AccountSettingsView_iOS {
    private func attemptDelete() async {
        do {
            ...
            if nsError.domain == AuthErrorDomain, nsError.code == AuthErrorCode.requiresRecentLogin.rawValue {
                // Show reauth sheet for email/password users
